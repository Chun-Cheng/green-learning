...
window.addEventListener('load', (event) => {
    let record_timer_id = setInterval(() => {
        ...
        if(read_history_title === null) {
            // create a record
            let new_read_history_title = '台灣的未來該「核」去何從？';
            ...
            for(let i = 0; i < read_history_title_list.length; i++) {
                if(read_history_title_list[i] === '台灣的未來該「核」去何從？') {
            ...
            if(!found) {
                // create a record
                // new_read_history_title = '台灣的未來該「核」去何從？';
                // new_read_history_duration = '5';
                // new_read_history_update = Date.now();//now_iso();
                read_history_title_list.push('台灣的未來該「核」去何從？');
